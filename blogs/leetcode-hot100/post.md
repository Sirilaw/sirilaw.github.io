...
全称是Double-Ended queue，是一种双端队列容器。支持头尾部的高效插入和删除，而vector只适合尾部操作。主要用于双端队列，[实现**滑动窗口**算法](#滑动窗口连续k个元素最大值239)等。

常见操作：

- 创建：`deque<int> dq`;
- 插入：`dq.push_front(1)`, `dq.push_back(1)`;
- 删除：`dq.pop_back(1)`, `dq.pop_front(1)`;
- 访问：`dq.at(0)`(会做边界检查)，`dq[0]`, `dq.back()`, `dq.front()`;

### sort()

头文件包含：`#include <algorithm>`。基本语法：`sort(begin, end)` or `sort(begin, end, comp)`。

使用自定义比较函数的例子：1）`sort(a.begin(), a.end(), std::greater<int>())`或者`sort(a.begin(), a.end(), [](const int& a, const int& b){return a > b;})`

### pair

头文件包含：`#include <utility>`。`p.first`和`p.second`。
...
双指针在算法应用中还蛮常见。根据做过的题目，大致分为以下几类：性质双指针、快慢双指针。
...
问题的核心在于，每次移动窗口的时候，如何以$O(1)$的时间复杂度得到当前窗口的最大值。如果遍历window，显然是行不通的。关键在于，如何在**丢掉一个元素，塞进一个元素**的同时，刻画出其对于最大值的影响。由于新加入的元素只需要和上一个窗口的最大值进行比较，所以如果丢掉的元素不是上一个最大值，则无需从队列里面删除；如果是的话，要删除，否则可能导致假阴。另外，新加入的元素，比它小的要丢掉，这样保持队列的**单调性**，从而保证队首的元素总是当前的最大值。

算法有两个核心点：1）保证队列里面的元素始终是窗口里面的；2）队列单调以便实现随机访问最大值。2）的细节，没啥说的。对于1），队列不同的实现思路会带来不一样的实现细节：

- 队列存的是下标：那么易如反掌了，把不在窗口范围的下标搞出去就行；
- 队列存的是值：那么根据之前的分析，如果front元素刚好就是丢出去的，那么赶紧从队列里面拿掉。
